...
export const SCRIPT_API: ApiEntry[] = [
  ...
  { name: 'DIR_SHORT', kind: 'variable', detail: 'Record<Direction, string>',
  ...
  { name: 'labelStyles', kind: 'function', signature: 'labelStyles(): { id, name, params }[]',
    detail: 'Read', info: "Label styles available to updateLabel({ style }). 'plain' is the unstyled default. params lists the style's own settings ({ id, name, type, default, options? }) for updateLabel({ styleParams }).", returns: 'unknown' },
...
export const LABEL_FIELDS: ApiEntry[] = [
  ...
  { name: 'styleParams', kind: 'variable', detail: 'Record<string, string | number | boolean>', info: "The label style's own settings, defaults filled in." },
...
export const ROOM_FIELDS: ApiEntry[] = [
  ...
  { name: 'customLines', kind: 'variable', detail: 'Record<string, { points, color, style, arrow }>',
