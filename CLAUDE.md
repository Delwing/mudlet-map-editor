...
- `src/editor/labelPixmap.ts` — labels are stored as PNGs, so every property change re-renders one. Text is laid out inside the box at its own font size (padding and border inset it — padding is per axis, a single number or `[horizontal, vertical]`, resolved through `resolveLabelPadding` and stored as `editor.labelPadding_N` = "8" or "8|4"), centred and measured on the glyphs' ink rather than their em boxes — `middleBaselineInkOffset` / `inkBlockHeight`, without which caps sit high in the box and "fit to text" leaves a bigger gap above and below than at the sides; resizing a label re-renders rather than stretching the bitmap, live during the drag up to `canRepaintLive`'s pixel budget. `labelStyles.ts` holds the per-style draw hooks (plugin `labelStyles()`): a style can own every stage — background, text, decoration, border — with the built-in version of each on `c.default` to reuse or skip, declare its own settings (`params`: enum/number/bool/color, rendered by the label panel, stored per label as `editor.labelStyleParams_N` JSON and handed back resolved as `c.params`), keep text off a shaped edge with `contentInsets` (honoured by the default layout, fit-to-text and auto-fit), and hide panel controls it has no use for with `uses`. The library ships no shapes or looks of its own beyond plain and caps; those belong in plugins. `labelPresets.ts` the named property bundles (plugin `labelPresets()`) that the label panel applies as one undo entry via `labelDiffCommands`. `labelPolicy.ts` holds the one setting that decides whether the pixmap or Mudlet's own label rendering wins (plugin `labelPolicy()`, defaults to Mudlet's, see {@link LabelPolicy}): Mudlet re-renders a label from its text whenever it has both text and a font family, which loses every style, border, padding and outline the editor drew, so `preservePixmaps` moves the real font to `editor.labelFont_N` and leaves `system.labelFont_N` family-less to fail that test. `supersample` renders the PNG larger than its nominal size for crisper text at zoom (`noScaling` labels excepted — Mudlet pins those to the pixmap's own pixel size), at roughly its square in bytes
